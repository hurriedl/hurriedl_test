	...
	bool prefiltered)
{
	...
	{
		compute_transmat(((float3*)orig_points)[idx], scales[idx], scale_modifier, rotations[idx], projmatrix, viewmatrix, W, H, T, s2w, normal);
		...
		float3 *s2w_ptr = (float3*)splat2worlds;
		s2w_ptr[idx * 3 + 0] = {s2w[0][0], s2w[0][1], s2w[0][2]}; //[列][行]
		s2w_ptr[idx * 3 + 1] = {s2w[1][0], s2w[1][1], s2w[1][2]};
		s2w_ptr[idx * 3 + 2] = {s2w[2][0], s2w[2][1], s2w[2][2]};
	} else {
...
renderCUDA(
	...
	const float* __restrict__ splat2worlds,
	...
	float* __restrict__ out_others)
{
	...
	__shared__ float3 collected_s2wu[BLOCK_SIZE];
	__shared__ float3 collected_s2wv[BLOCK_SIZE];
	__shared__ float3 collected_s2ww[BLOCK_SIZE];
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if (range.x + progress < range.y)
		{
			...
			collected_s2wu[block.thread_rank()] = {splat2worlds[9 * coll_id+0], splat2worlds[9 * coll_id+1], splat2worlds[9 * coll_id+2]};
			collected_s2wv[block.thread_rank()] = {splat2worlds[9 * coll_id+3], splat2worlds[9 * coll_id+4], splat2worlds[9 * coll_id+5]};
			collected_s2ww[block.thread_rank()] = {splat2worlds[9 * coll_id+6], splat2worlds[9 * coll_id+7], splat2worlds[9 * coll_id+8]};
		}
		block.sync();
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			const float3 s2wu = collected_s2wu[j];
			const float3 s2wv = collected_s2wv[j];
			const float3 s2ww = collected_s2ww[j];
			...
			if (ortho)
			{
				...
				p_world.x = s.x * s2wu.x + s.y * s2wv.x + s2ww.x;
				p_world.y = s.x * s2wu.y + s.y * s2wv.y +s2ww.y;
				p_world.z = s.x * s2wu.z + s.y * s2wv.z +s2ww.z;

				float3 c0_world;
				c0_world.x = (*cam_pos).x;
				c0_world.y = (*cam_pos).y;
				...
				float3 p_c1;
				p_c1.x = c1_world.x-p_world.x;
				p_c1.y = c1_world.y-p_world.y;
				p_c1.z = c1_world.z-p_world.z;

				//float depth = sqrtf(p_c1.x*p_c1.x+p_c1.y*p_c1.y+p_c1.z*p_c1.z);
				depth = (rho3d <= rho2d) ? sqrtf(p_c1.x*p_c1.x+p_c1.y*p_c1.y+p_c1.z*p_c1.z) : s2ww.z;
			}
			else
			{
