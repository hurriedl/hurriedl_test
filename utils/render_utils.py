...
def focus_point_fn(poses: np.ndarray) -> np.ndarray:
  ...
  focus_pt = np.linalg.pinv(mt_m.mean(0)) @ (mt_m @ origins).mean(0)[:, 0] # TODO:暂时用伪逆矩阵代替
  return focus_pt
...
def save_depth_with_label(depth_map, pth, min_depth, max_depth, idx):
  # 绘制图例
  im_color_rgb = cv2.cvtColor(depth_map, cv2.COLOR_BGR2RGB)
  ...
  sm.set_array([])  # 只需要一个空数组
  plt.colorbar(sm, orientation='vertical', label='Depth')
  
  plt.savefig(os.path.join(pth, 'depth_color_{0:05d}'.format(idx) + ".png"))
  plt.close()